...
async function createSchemas() {
  ...
  if (!existingSlugs.includes('contacts')) {
    await client.collections.create({
      ...
      properties: [
        ...
        { propertyName: 'Tasks', systemName: 'tasks', type: 'array', autoSystem: true, updateSemantics: 'append', description: 'Action items for this lead. Each entry: { title, description, status (pending|in_progress|done|cancelled), owner (outreach-agent|enrichment-agent|sales-rep), priority (low|medium|high|urgent), dueDate, outcome }. Tasks can be updated as they progress.' },
        ...
        { propertyName: 'Issues', systemName: 'issues', type: 'array', autoSystem: true, updateSemantics: 'append', description: 'Problems, risks, and blockers. Each entry: { title, description, severity (low|medium|high|critical), status (open|resolved|dismissed), raisedBy, resolution, timestamp }. Bounces, opt-outs, competitor mentions — all flagged here.' },
        { propertyName: 'Messages Sent', systemName: 'messages_sent', type: 'array', autoSystem: true, updateSemantics: 'append', description: 'Every outreach message sent. Each entry: { channel (email|call|linkedin), subject, bodyPreview, step, angle, sentBy, status (sent|delivered|opened|clicked|replied|bounced), sentAt }. The definitive record of what was communicated.' },
      ],
    });
  ...
  if (!existingSlugs.includes('companies')) {
    ...
    logger.info('Created Companies collection');
  } else {
